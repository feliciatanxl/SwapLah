...
ACTIVE_LISTINGS_SQL = """
SELECT
    listings.id, listings.title, listings.description, listings.price,
    listings.category, listings.item_condition AS condition,
    listings.image_url, listings.listing_date, users.display_name AS seller
FROM listings
...
LISTING_DETAIL_SQL = """
SELECT
    listings.id, listings.seller_id, listings.title, listings.description,
...
UPDATE listings
SET status = 'Deleted', last_modified_timestamp = ?
WHERE id = ? AND seller_id = ? AND status = 'Active'
"""
...
SELECT
    offers.id, offers.listing_id, offers.buyer_id, offers.offer_type,
...
SELECT
    transactions.id, transactions.transaction_type, transactions.amount,
...
GET_ALL_REPORTS_SQL = """
SELECT
    r.id, r.listing_id, r.reporter_id, r.reason, r.description,
...
def get_sold_listings_by_seller(seller_id):
    ...
    rows = conn.execute(
        ...
        SELECT listings.id, listings.title, listings.description, listings.price,
               listings.category, listings.item_condition AS condition,
               listings.image_url, offers.created_at AS sold_at,
               offers.offer_type, offers.proposed_price, users.display_name AS buyer
        FROM offers
...
def get_active_listing_by_buyer(listing_id, buyer_id):
    """Return listing if active and not owned by buyer (for offer creation)."""
    conn = get_db_connection()
    row = conn.execute(
        "SELECT * FROM listings WHERE id = ? AND status = 'Active' AND seller_id != ?",
        (listing_id, buyer_id)
    ).fetchone()
    conn.close()
...
def create_report(listing_id, reporter_id, reason, description=None):
    ...
    try:
        ...
        cursor = conn.execute(CREATE_REPORT_SQL, (listing_id, reporter_id, reason, cleaned_description))
        conn.commit()
