...
INSERT_LISTING_SQL = """INSERT INTO listings (seller_id,title,description,price,category,item_condition,image_url,listing_date,last_modified_timestamp) VALUES (?,?,?,?,?,?,?,?,?)"""

ACTIVE_LISTINGS_SQL = """SELECT listings.id,listings.title,listings.description,listings.price,listings.category,listings.item_condition AS condition,listings.image_url,listings.listing_date,users.display_name AS seller FROM listings LEFT JOIN users ON listings.seller_id = users.id WHERE listings.status='Active' ORDER BY listings.listing_date DESC"""

LISTING_DETAIL_SQL = """SELECT listings.id,listings.seller_id,listings.title,listings.description,listings.price,listings.category,listings.item_condition AS condition,listings.image_url,listings.listing_date,listings.last_modified_timestamp,users.display_name AS seller_display_name,users.email AS seller_email,users.contact_number AS seller_contact_number FROM listings LEFT JOIN users ON listings.seller_id = users.id WHERE listings.id=? AND listings.status='Active'"""

UPDATE_LISTING_SQL = """UPDATE listings SET title=?, description=?, price=?, category=?, item_condition=?, image_url=?, last_modified_timestamp=? WHERE id=? AND seller_id=? AND status='Active'"""

SOFT_DELETE_LISTING_SQL = """UPDATE listings SET status='Deleted', last_modified_timestamp=? WHERE id=? AND seller_id=? AND status='Active'"""

GET_USER_BY_ID_SQL = """SELECT id,student_id,first_name,last_name,display_name,email,contact_number,role,status,created_at FROM users WHERE id=:user_id"""

UPDATE_USER_WITH_PASSWORD_SQL = """UPDATE users SET first_name=:first_name, last_name=:last_name, display_name=:display_name, contact_number=:contact_number, password_hash=:password_hash WHERE id=:user_id"""

UPDATE_USER_SQL = """UPDATE users SET first_name=:first_name, last_name=:last_name, display_name=:display_name, contact_number=:contact_number WHERE id=:user_id"""

CREATE_REPORT_SQL = """INSERT INTO reports (listing_id, reporter_id, reason, description) VALUES (?,?,?,?)"""

GET_REPORT_BY_ID_SQL = """SELECT r.*, l.title, l.status as listing_status, l.seller_id FROM reports r JOIN listings l ON r.listing_id = l.id WHERE r.id=?"""

ADMIN_DELETE_REPORTED_LISTING_SQL = """UPDATE listings SET status='Deleted', last_modified_timestamp=? WHERE id=? AND status='Active'"""

RESOLVE_REPORT_SQL = """UPDATE reports SET status='Resolved' WHERE id=?"""

OFFERS_FOR_SELLER_SQL = """SELECT offers.id,offers.listing_id,offers.buyer_id,offers.offer_type,offers.proposed_price,offers.swap_listing_id,offers.status,offers.created_at,listings.title AS listing_title,listings.category AS listing_category,listings.price AS listing_price,buyer.display_name AS buyer_display_name,swap_listing.title AS swap_listing_title FROM offers JOIN listings ON offers.listing_id=listings.id JOIN users AS buyer ON offers.buyer_id=buyer.id LEFT JOIN listings AS swap_listing ON offers.swap_listing_id=swap_listing.id WHERE listings.seller_id=? ORDER BY offers.created_at DESC"""

TRANSACTIONS_FOR_BUYER_SQL = """SELECT transactions.id,transactions.transaction_type,transactions.amount,transactions.created_at,listings.title AS listing_title,listings.category AS listing_category,seller.display_name AS counterparty_display_name FROM transactions JOIN listings ON transactions.listing_id=listings.id JOIN users AS seller ON transactions.seller_id=seller.id WHERE transactions.buyer_id=? ORDER BY transactions.created_at DESC"""

TRANSACTIONS_FOR_SELLER_SQL = """SELECT transactions.id,transactions.transaction_type,transactions.amount,transactions.created_at,listings.title AS listing_title,listings.category AS listing_category,buyer.display_name AS counterparty_display_name FROM transactions JOIN listings ON transactions.listing_id=listings.id JOIN users AS buyer ON transactions.buyer_id=buyer.id WHERE transactions.seller_id=? ORDER BY transactions.created_at DESC"""

GET_ALL_REPORTS_SQL = """SELECT r.id,r.listing_id,r.reporter_id,r.reason,r.description,r.status,r.created_at,COALESCE(l.title,'Deleted Listing') as listing_title,COALESCE(l.category,'Unknown') as listing_category,COALESCE(u.display_name,'Unknown User') as reporter_display_name FROM reports r LEFT JOIN listings l ON r.listing_id=l.id LEFT JOIN users u ON r.reporter_id=u.id ORDER BY r.created_at DESC"""
...
def get_listing_category_summary():
    conn = get_db_connection()
    rows = conn.execute("SELECT category, COUNT(*) AS count FROM listings WHERE status='Active' GROUP BY category").fetchall()
    ...
    return {
        ...
        "category_rows": [{"label": cat["label"], "icon": cat["icon"], "count": category_counts.get(cat["label"], 0)} for cat in LISTING_CATEGORIES]
    }
...
def _get_active_listing(conn, listing_id):
    return conn.execute("SELECT * FROM listings WHERE id=? AND status='Active'", (listing_id,)).fetchone()

def _listing_update_values(listing_id, seller_id, fields):
    return (fields["title"], fields["description"], fields["price"], fields["category"], fields["condition"], fields["image_url"], _now(), listing_id, seller_id)
...
def update_listing(listing_id, seller_id, title, description, price, category, condition, image_url):
    ...
    fields = {"title": title, "description": description, "price": price, "category": category, "condition": condition, "image_url": image_url}
...
def get_listings_by_seller(seller_id):
    conn = get_db_connection()
    rows = conn.execute("SELECT id, title FROM listings WHERE seller_id=? AND status='Active'", (seller_id,)).fetchall()
    conn.close()
    return [dict(row) for row in rows]

def get_active_listings_by_seller(seller_id):
    conn = get_db_connection()
    rows = conn.execute("SELECT id,title,description,price,category,item_condition AS condition,image_url,listing_date FROM listings WHERE seller_id=? AND status='Active' ORDER BY listing_date DESC", (seller_id,)).fetchall()
    conn.close()
    return [_attach_images(dict(row)) for row in rows]

def get_sold_listings_by_seller(seller_id):
    conn = get_db_connection()
    rows = conn.execute("SELECT listings.id,listings.title,listings.description,listings.price,listings.category,listings.item_condition AS condition,listings.image_url,offers.created_at AS sold_at,offers.offer_type,offers.proposed_price,users.display_name AS buyer FROM offers INNER JOIN listings ON offers.listing_id=listings.id LEFT JOIN users ON offers.buyer_id=users.id WHERE listings.seller_id=? AND offers.status='Accepted' ORDER BY offers.created_at DESC", (seller_id,)).fetchall()
    conn.close()
    return [_attach_images(dict(row)) for row in rows]

def get_active_listing_by_buyer(listing_id, buyer_id):
    conn = get_db_connection()
    row = conn.execute("SELECT * FROM listings WHERE id=? AND seller_id=? AND status='Active'", (listing_id, buyer_id)).fetchone()
    conn.close()
...
def get_active_listing_by_seller(listing_id, seller_id):
    ...
    conn = get_db_connection()
    row = conn.execute("SELECT * FROM listings WHERE id=? AND seller_id=? AND status='Active'", (listing_id, seller_id)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def _active_listing_search_filters(keyword="", category="", condition=""):
    ...
    if search:
        pattern = f"%{search}%"
        clauses.append("(LOWER(listings.title) LIKE LOWER(?) OR LOWER(listings.description) LIKE LOWER(?))")
...
def _active_listing_search_sql(where_clause):
    return f"SELECT listings.id,listings.title,listings.description,listings.price,listings.category,listings.item_condition AS condition,listings.image_url,listings.listing_date,users.display_name AS seller FROM listings LEFT JOIN users ON listings.seller_id=users.id WHERE {where_clause} ORDER BY listings.listing_date DESC"
...
def _profile_stats_sql():
    return "SELECT COUNT(DISTINCT CASE WHEN listings.status='Active' THEN listings.id END) AS active_count, COUNT(DISTINCT CASE WHEN offers.status='Accepted' THEN offers.id END) AS total_sales, COUNT(DISTINCT offers.id) AS offer_count, COUNT(DISTINCT CASE WHEN offers.status!='Pending' THEN offers.id END) AS responded_offer_count, COUNT(DISTINCT reviews.id) AS review_count, AVG(reviews.rating) AS average_rating FROM users LEFT JOIN listings ON listings.seller_id=users.id LEFT JOIN offers ON offers.listing_id=listings.id LEFT JOIN reviews ON reviews.reviewed_user_id=users.id WHERE users.id=?"
...
def _trust_badge(total_sales, active_count):
    if total_sales >= 20: return "Top Seller"
    if total_sales >= 5: return "Trusted Seller"
    if active_count > 0: return "Active Seller"
    return "New Seller"

def _response_rate(row):
    offer_count = row["offer_count"] or 0
    if not offer_count: return None
...
def get_reviews_for_user(user_id):
    conn = get_db_connection()
    rows = conn.execute("SELECT reviews.id,reviews.reviewed_user_id,reviews.reviewer_id,reviews.rating,reviews.comment,reviews.created_at,users.display_name AS reviewer_display_name FROM reviews LEFT JOIN users ON reviews.reviewer_id=users.id WHERE reviews.reviewed_user_id=? ORDER BY reviews.created_at DESC, reviews.id DESC", (user_id,)).fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def update_user_account(user_id, first_name, last_name, display_name, contact_number, password_hash=None):
    ...
    profile = {"first_name": first_name, "last_name": last_name, "display_name": display_name, "contact_number": contact_number, "user_id": user_id}
...
def create_offer(listing_id, buyer_id, offer_type, proposed_price=None, swap_listing_id=None):
    conn = get_db_connection()
    cursor = conn.execute("INSERT INTO offers (listing_id,buyer_id,offer_type,proposed_price,swap_listing_id,status,created_at) VALUES (?,?,?,?,?,'Pending',?)", (listing_id, buyer_id, offer_type, proposed_price, swap_listing_id, _now()))
    conn.commit()
...
def _create_transaction_for_offer(conn, offer):
    seller_id = conn.execute("SELECT seller_id FROM listings WHERE id=?", (offer["listing_id"],)).fetchone()["seller_id"]
    conn.execute("INSERT INTO transactions (offer_id,listing_id,seller_id,buyer_id,transaction_type,amount,swap_listing_id) VALUES (?,?,?,?,?,?,?)", (offer["id"], offer["listing_id"], seller_id, offer["buyer_id"], offer["offer_type"], offer["proposed_price"], offer["swap_listing_id"]))
...
def accept_offer(offer_id):
    ...
    conn.execute("UPDATE offers SET status='Rejected' WHERE listing_id=? AND id!=? AND status='Pending'", (offer["listing_id"], offer_id))
...
def _validate_report_reason(reason):
    allowed = ["Counterfeit","Prohibited item","Spam","Inappropriate content","Other","Suspected counterfeit","Copyright violation","Unsafe or inappropriate"]
...
def _ensure_no_duplicate_report(conn, listing_id, reporter_id):
    existing = conn.execute("SELECT id FROM reports WHERE listing_id=? AND reporter_id=?", (listing_id, reporter_id)).fetchone()
    if existing:
...
def _get_pending_report_for_deletion(conn, report_id):
    report = conn.execute("SELECT id,listing_id,status FROM reports WHERE id=?", (report_id,)).fetchone()
    if not report or report["status"] != "Pending":
        return None
    return report

def _get_active_listing_for_report(conn, listing_id):
    listing = conn.execute("SELECT id,status FROM listings WHERE id=?", (listing_id,)).fetchone()
