"""
API integration tests for POST /api/transactions/<id>/review (User Story #38).
...
def client(tmp_path, monkeypatch):
    test_db = tmp_path / "test_seller_review.db"
...
# ===========================================================================
# AC1: seller submits a valid 1-5 rating with optional comment for the buyer
# ===========================================================================

def test_seller_can_submit_review_with_comment(client):
    """AC1: seller leaves a rating + comment for the buyer on a completed deal."""
    ...
    resp = test_client.post(
        f"/api/transactions/{transaction_id}/review",
        json={"rating": 5, "comment": "Prompt payment, smooth handover."},
    )
    ...
    assert data["review"]["reviewedUserId"] == buyer_id
    assert data["review"]["reviewerId"] == seller_id
    assert data["review"]["transactionId"] == transaction_id


def test_seller_can_submit_review_without_comment(client):
...
def test_review_rejected_for_pending_offer_with_no_transaction(client):
    ...
    test_client, test_db = client
    seller_id, _buyer_id = seed_pending_offer(test_db)
    login_as(test_client, seller_id)

    resp = test_client.post("/api/transactions/999999/review", json={"rating": 5})
...
# ===========================================================================
# AC3: review is linked to the completed transaction and the buyer's profile
# ===========================================================================

def test_review_is_linked_to_transaction_and_buyer_profile(client):
    """AC3: the saved review is retrievable from the buyer's public review list."""
    test_client, test_db = client
    seller_id, buyer_id, transaction_id = seed_completed_deal(test_db)
    login_as(test_client, seller_id)
    ...
    test_client.post(
        f"/api/transactions/{transaction_id}/review",
        json={"rating": 5, "comment": "Great buyer."},
    )

    resp = test_client.get(f"/api/users/{buyer_id}/reviews")

    assert resp.status_code == 200
    reviews = resp.get_json()["reviews"]
    assert len(reviews) == 1
    assert reviews[0]["reviewed_user_id"] == buyer_id
    assert reviews[0]["reviewer_id"] == seller_id
...
def test_review_rejects_missing_rating(client):
    ...
    resp = test_client.post(f"/api/transactions/{transaction_id}/review", json={})
...
def test_buyer_cannot_submit_seller_review(client):
    """AC5: the buyer of the transaction is not its seller -> rejected."""
    test_client, test_db = client
    _seller_id, buyer_id, transaction_id = seed_completed_deal(test_db)
    login_as(test_client, buyer_id)

    resp = test_client.post(
        f"/api/transactions/{transaction_id}/review", json={"rating": 5}
    )
...
def test_unrelated_user_cannot_submit_seller_review(client):
...
def test_unauthenticated_user_cannot_submit_seller_review(client):
